...
import com.trc.pndao.generator.dao.DaoEnv;
import com.trc.pndao.generator.dao.MapperMethod;
...
public class DaoGenHelper {
    ...
    private BiConsumer<String, MapperMethod> getGenFunc(DaoGen daoGen, Element root) {
        return (key, method) -> {
            if (!(handledWithThisPrefix(key, daoGen::insertPrefix, addInsert(daoGen, key, method, root))
                    ...
                    || handledWithThisPrefix(key, daoGen::getListPrefix, addList(daoGen, key, method, root))
            )) {
    ...
    private void appendParams(List<String> params,Element sql){
        for (String param : params) {
            String realParam = lowerFirst(param);
            ...
            ifText.append(" And ")
                    ...
                    .append(realParam)
                    .append("}");
            ifTest.addText(ifText.toString());
        }
    }
